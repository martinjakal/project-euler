...
bool findNextLink(const std::vector<std::vector<int>>& figurate, int idx, std::vector<int>& chain)
{
    for (auto number : figurate[idx])
    {
        ...
        if (chain.size() == figurate.size() && number % 100 == chain[0] / 100)
            return true;
...
int sumCyclicalFigurateNumbers()
{
    ...
    // Different permutations of number sets are checked. Starting from the first set, use backtracking to find 
